...
export default function AdminDashboard() {
  ...
  // Teacher Assignment per Student Classroom States & Functions
  const [selectedTeacherMap, setSelectedTeacherMap] = useState({})
  const [savingTeacherAssignment, setSavingTeacherAssignment] = useState({})

  const getEligibleTeachersForCourse = (courseId) => {
    if (!courseId) return approvedTeachers
    const targetCourse = courses.find((c) => String(c._id) === String(courseId))
    if (!targetCourse) return approvedTeachers

    const explicitIds = new Set()
    if (targetCourse.educator) {
      explicitIds.add(typeof targetCourse.educator === 'object' ? String(targetCourse.educator._id) : String(targetCourse.educator))
    }
    if (Array.isArray(targetCourse.instructors)) {
      targetCourse.instructors.forEach((inst) => {
        explicitIds.add(typeof inst === 'object' ? String(inst._id) : String(inst))
      })
    }

    const eligible = approvedTeachers.filter((t) => {
      const isExplicit = explicitIds.has(String(t._id))
      const intendedId = t.intendedCourse?._id || t.intendedCourse
      const isIntended = intendedId && String(intendedId) === String(courseId)
      return isExplicit || isIntended
    })

    return eligible.length > 0 ? eligible : approvedTeachers
  }

  const handleAssignTeacherToStudentClassroom = async (classroomId, newTeacherId) => {
    if (!classroomId || !newTeacherId) return
    setSavingTeacherAssignment((prev) => ({ ...prev, [classroomId]: true }))
    try {
      const res = await api.put(`/admin/classrooms/${classroomId}/assign-teacher`, { teacherId: newTeacherId })
      toast.success(res.data?.message || 'Instructor assigned successfully!')
      await loadData()
    } catch (err) {
                  ...
                  if (entry) {
                    if (!entry.classrooms.some((c) => c.classroomId === cr._id)) {
                      entry.classrooms.push({
                        classroomId: cr._id,
                        courseId: cr.course?._id || cr.course || '',
                        courseTitle: cr.course?.title || 'Untitled Course',
                        courseThumbnail: cr.course?.thumbnailImage || '',
                        teacherId: cr.teacher?._id || cr.teacher || '',
                        teacherName: cr.teacher?.name || 'Unassigned',
                        assignedTeacherId: cr.teacher?._id || cr.teacher || '',
                        assignedTeacherName: cr.teacher?.name || 'Unassigned',
                              ...
                              >
                                ...
                                <div className="mt-3 pt-3 border-t border-slate-100" onClick={(e) => e.stopPropagation()}>
                                  <div className="flex items-center justify-between text-xs mb-1.5">
                                    <span className="font-semibold text-slate-500">Current Instructor:</span>
                                  ...
                                  <div className="flex items-center gap-2">
                                    <select
                                      value={
                                        selectedTeacherMap[cr.classroomId] !== undefined
                                          ? selectedTeacherMap[cr.classroomId]
                                          : (cr.assignedTeacherId || '')
                                      }
                                      onChange={(e) =>
                                        setSelectedTeacherMap({
                                          ...selectedTeacherMap,
                                          [cr.classroomId]: e.target.value,
                                        })
                                      }
                                      className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1.5 text-xs font-semibold text-slate-900 focus:ring-2 focus:ring-blue-500 focus:outline-none cursor-pointer"
                                    >
                                      <option value="">Select Eligible Teacher...</option>
                                      {getEligibleTeachersForCourse(cr.courseId).map((t) => (
                                    ...
                                    <button
                                      disabled={
                                        savingTeacherAssignment[cr.classroomId] ||
                                        (selectedTeacherMap[cr.classroomId] !== undefined
                                          ? selectedTeacherMap[cr.classroomId] === cr.assignedTeacherId
                                          : true) ||
                                        !selectedTeacherMap[cr.classroomId]
                                      }
                                      onClick={() =>
                                        handleAssignTeacherToStudentClassroom(
                                          cr.classroomId,
                                          selectedTeacherMap[cr.classroomId]
                                        )
                                      }
                                      className="px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-sm shrink-0"
                                    >
                                      {savingTeacherAssignment[cr.classroomId] ? 'Saving...' : 'Assign Teacher'}
                                    </button>
                                  </div>
                                </div>
